...
if(Yii::$app->user !== null && Yii::$app->user->identity !== null)
{
    ...
    if(Yii::$app->authManager->checkAccess(Yii::$app->user->getId(),'province/list')
    ...
    if(Yii::$app->authManager->checkAccess(Yii::$app->user->getId(),'parroquia/list')
        || Yii::$app->user->identity->getIsAdmin())
    {
        $localization['items'][]  =['label' => 'Parroquias', 'icon' => 'bank', 'url' => ['/parroquia/index']];
    }

    if(Yii::$app->authManager->checkAccess(Yii::$app->user->getId(),'zona/list')
        || Yii::$app->user->identity->getIsAdmin())
    {
        $localization['items'][]=['label' => 'Zonas', 'icon' => 'map-signs', 'url' => ['/zona/index']];
    }

    if(Yii::$app->authManager->checkAccess(Yii::$app->user->getId(),'recinto/list')
        || Yii::$app->user->identity->getIsAdmin())
    {
...
// Menus que tienen más relacion con la el proceso eleccionario
    if(Yii::$app->authManager->checkAccess(Yii::$app->user->getId(),'eleccion/list')
        || Yii::$app->user->identity->getIsAdmin())
    {
    ...
    if(Yii::$app->authManager->checkAccess(Yii::$app->user->getId(),'juntas/list')
        || Yii::$app->user->identity->getIsAdmin())
    {
        $eleccion['items'][]=['label' => 'Juntas', 'icon' => 'archive', 'url' => ['/junta/index']];
    }

    if(Yii::$app->authManager->checkAccess(Yii::$app->user->getId(),'voto/list') ||
    ...
    if(Yii::$app->authManager->checkAccess(Yii::$app->user->getId(),'partido/list')
        || Yii::$app->user->identity->getIsAdmin())
    {
        $eleccion['items'][]=['label' => 'Partido', 'icon' => 'object-group', 'url' => ['/partido/index']];
    }

    if(Yii::$app->authManager->checkAccess(Yii::$app->user->getId(),'persona/list')
        || Yii::$app->user->identity->getIsAdmin())
    {
